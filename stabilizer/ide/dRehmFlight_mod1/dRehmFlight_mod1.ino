...
unsigned long ch1_fs = 1000; //thro
unsigned long ch2_fs = 1500; //ail
unsigned long ch3_fs = 1500; //elev
unsigned long ch4_fs = 1500; //rudd
unsigned long ch5_fs = 2000; //gear, greater than 1500 = throttle cut
unsigned long ch6_fs = 2000; //aux1
...
//Radio comm:
unsigned long ch1_in_pwm, ch2_in_pwm, ch3_in_pwm, ch4_in_pwm, ch5_in_pwm, ch6_in_pwm;
unsigned long ch1_in_pwm_prev, ch2_in_pwm_prev, ch3_in_pwm_prev, ch4_in_pwm_prev;
...
void getDesState() {
   ...
   */
  thro_des = (ch1_in_pwm - 1000.0)/1000.0; //between 0 and 1
  roll_des = (ch2_in_pwm - 1500.0)/500.0; //between -1 and 1
  pitch_des = (ch3_in_pwm - 1500.0)/500.0; //between -1 and 1
  yaw_des = (ch4_in_pwm - 1500.0)/500.0; //between -1 and 1
...
void controlANGLE() {
  ...
  if (ch1_in_pwm < 1060) {   //don't let integrator build if throttle is too low
    integral_roll = 0;
  }
...
void controlMixer() {
  ...
  /*
  if (ch6_in_pwm > 1500){ //go to max specified value in 5.5 seconds
  ...
  if (ch6_in_pwm < 1500) { //go to min specified value in 2.5 seconds
...
void getCommands() {
   ...
   */
    ...
    ch1_in_pwm = getRadioPWM(1);
    ch2_in_pwm = getRadioPWM(2);
    ch3_in_pwm = getRadioPWM(3);
    ch4_in_pwm = getRadioPWM(4);
    ch5_in_pwm = getRadioPWM(5);
    ch6_in_pwm = getRadioPWM(6);
    ...
    if (sbus.read(&sbusChannels[0], &sbusFailSafe, &sbusLostFrame))
    {
      ...
      float scale = 0.615;  
      float bias  = 895.0; 
      ch1_in_pwm = sbusChannels[0] * scale + bias;
      ch2_in_pwm = sbusChannels[1] * scale + bias;
      ch3_in_pwm = sbusChannels[2] * scale + bias;
      ch4_in_pwm = sbusChannels[3] * scale + bias;
      ch5_in_pwm = sbusChannels[4] * scale + bias;
      ch6_in_pwm = sbusChannels[5] * scale + bias; 
    }
  #endif
  
  //Low-pass the critical commands and update previous values
  float b = 0.2; //lower=slower, higher=noiser
  ch1_in_pwm = (1.0 - b)*ch1_in_pwm_prev + b*ch1_in_pwm;
  ch2_in_pwm = (1.0 - b)*ch2_in_pwm_prev + b*ch2_in_pwm;
  ch3_in_pwm = (1.0 - b)*ch3_in_pwm_prev + b*ch3_in_pwm;
  ch4_in_pwm = (1.0 - b)*ch4_in_pwm_prev + b*ch4_in_pwm;
  ch1_in_pwm_prev = ch1_in_pwm;
  ch2_in_pwm_prev = ch2_in_pwm;
  ch3_in_pwm_prev = ch3_in_pwm;
  ch4_in_pwm_prev = ch4_in_pwm;
}
...
void failSafe() {
  ...
  if (ch1_in_pwm > maxVal || ch1_in_pwm < minVal) check1 = 1;
  if (ch2_in_pwm > maxVal || ch2_in_pwm < minVal) check2 = 1;
  if (ch3_in_pwm > maxVal || ch3_in_pwm < minVal) check3 = 1;
  if (ch4_in_pwm > maxVal || ch4_in_pwm < minVal) check4 = 1;
  if (ch5_in_pwm > maxVal || ch5_in_pwm < minVal) check5 = 1;
  if (ch6_in_pwm > maxVal || ch6_in_pwm < minVal) check6 = 1;
...
float floatFaderLinear(float param, float param_min, float param_max, float fadeTime, int state, int loopFreq){
   ...
   *  and high/low states can be determined by monitoring the state of an auxillarly radio channel. For example, if ch6_in_pwm is being 
...
void throttleCut() {
  /*
   * Monitors the state of radio command ch5_in_pwm and directly sets the mx_command_PWM values to minimum (120 is
...
void printRadioData() {
  if (current_time - print_counter > 10000) {
    ...
    Serial.print(F(" CH1: "));
    Serial.print(ch1_in_pwm);
    Serial.print(F(" CH2: "));
    Serial.print(ch2_in_pwm);
    Serial.print(F(" CH3: "));
    Serial.print(ch3_in_pwm);
    Serial.print(F(" CH4: "));
    Serial.print(ch4_in_pwm);
    Serial.print(F(" CH5: "));
    Serial.print(ch5_in_pwm);
    Serial.print(F(" CH6: "));
    Serial.println(ch6_in_pwm);
  }
}
